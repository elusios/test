...
UPDATE posts
SET client_id = FLOOR(RAND() * 11) + 1;

SET FOREIGN_KEY_CHECKS = 1;

SELECT posts.title, posts.full_text, posts.client_id, clients.login
FROM posts, clients
WHERE posts.client_id = clients.id;

SELECT posts.title, posts.full_text
FROM posts, clients
WHERE posts.client_id = clients.id AND clients.login = "Mikle";